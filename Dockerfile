...
set -e
cargo build --locked --release --bin template-server --bin template-consumer
cp ./target/release/template-server /bin/template-server
cp ./target/release/template-consumer /bin/template-consumer
...
FROM debian:bookworm-slim AS final-consumer
...
COPY --from=build /bin/template-consumer /bin/

# What the container should run when it is started.
CMD ["/bin/template-consumer"]
